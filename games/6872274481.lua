...
run(function()
	local BlurredTextured
	local originalProperties = {}
	local blurMonitorConnections = {}
	local processedBlocks = {}
	local cachedColors = {}
	...
	local function blurBlock(block)
		if not block or not block.Parent or processedBlocks[block] then return end
		if not originalProperties[block] then
			originalProperties[block] = {
				Material = block.Material,
	...
	local function processExistingBlurred(blur)
		local descendants = workspace:GetDescendants()
		task.spawn(function()
			for i, obj in descendants do
				if isTargetBlock(obj) then
	...
	local function setupBlurMonitor(blur)
		for _, conn in blurMonitorConnections do
			conn:Disconnect()
		end
		table.clear(blurMonitorConnections)
		if not blur then return end
		table.insert(blurMonitorConnections, workspace.DescendantAdded:Connect(function(descendant)
			if isTargetBlock(descendant) then
				task.defer(function()
					if descendant and descendant.Parent then
						blurBlock(descendant)
					end
				end)
			end
		end))
		local lastCleanup = 0
		table.insert(blurMonitorConnections, runService.Heartbeat:Connect(function()
			local now = tick()
			if now - lastCleanup >= 5 then
				lastCleanup = now
				cleanupDeadReferences()
	...
	BlurredTextured = larp.Categories.Utility:CreateModule({
		Name = 'Blurred Textured',
		Function = function(callback)
			if callback then
				processExistingBlurred(true)
				setupBlurMonitor(true)
			else
				processExistingBlurred(false)
				for _, conn in blurMonitorConnections do
					conn:Disconnect()
				end
				table.clear(blurMonitorConnections)
		...
		Tooltip = 'Blurs block textures and decals'
	})
end)
