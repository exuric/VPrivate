...
local function downloadSplit(base)
	...
	for i = 0, 1 do
		local ok, res = pcall(function()
			return game:HttpGet(ROOT..readfile('LarpV4/profiles/commit.txt')..'/'..select(1, base:gsub('^LarpV4/', ''))..'.'..i..'?v=120', true)
		end)
		...
		if (not teleportedServers) and (not shared.LarpIndependent) then
			...
			local teleportScript = [[
				...
				if shared.LarpDeveloper then
					...
					loadstring(game:HttpGet(']]..ROOT..[['..readfile('LarpV4/profiles/commit.txt')..'/init120.lua?v=120', true), 'init')(_scriptconfig)
				end
			]]
...
larp = loadstring(downloadFile('LarpV4/guis/larp120.lua'), 'gui')(license)
if type(larp) ~= 'table' then
	error('larp120.lua did not return a valid api table' .. (larp and ': '..tostring(larp) or ''))
end
shared.larp = larp
_G.larp = larp
getgenv().larp = larp
...
if not shared.LarpIndependent then
	loadstring(downloadFile('LarpV4/games/universal120.lua'), 'universal')(license)
	if isfile('LarpV4/games/'..game.PlaceId..'_120.lua') then
		loadstring(readfile('LarpV4/games/'..game.PlaceId..'_120.lua'), tostring(game.PlaceId))(license)
	else
		if not shared.LarpDeveloper then
			local suc, res = pcall(function()
				return game:HttpGet(ROOT..readfile('LarpV4/profiles/commit.txt')..'/games/'..game.PlaceId..'_120.lua?v=120', true)
			end)
			if suc and res ~= '404: Not Found' then
				loadstring(downloadFile('LarpV4/games/'..game.PlaceId..'_120.lua'), tostring(game.PlaceId))(license)
			end
		end
	end
	finishLoading()
