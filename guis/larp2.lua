...
do
	local data = isfile('LarpV4/profiles/color.txt') and loadJson('LarpV4/profiles/color.txt')
	if data then
...
local getfontsize = getfontbounds
...
do
		...
		if not cached or (path:find('.lua') and #cached < 100) then
			...
			if _pending[path] then
				...
				if path:find('.lua') then
					res = '--This watermark is used to delete the file if its cached, remove it to make the file persist after larp updates.`r`n'..res
				end
				writefile(path, res)
			end
		else
	...
	vape.Libraries.getcustomasset = getcustomasset
	vape.getcustomasset = getcustomasset
end
...
function vape:Load(skipgui, profile)
	...
	if isfile('LarpV4/profiles/'..self.Profile..self.Place..'.txt') then
		local mainData = loadJson('LarpV4/profiles/'..self.Profile..self.Place..'.txt')
...
function vape:UpdateFavourites()
	local favcategory = self.Categories.Favorites
	if not favcategory then return end
	local favchildren
	local favobject = favcategory.Object
	if favobject then
		favchildren = favobject:FindFirstChild('Children')
		if favchildren and not favchildren:IsA('ScrollingFrame') then
			favchildren = favchildren:FindFirstChildOfClass('ScrollingFrame') or favchildren
		end
	end
	if not favchildren then return end
	local count = 0
	for _, moduleapi in pairs(self.Modules) do
		local favopt = moduleapi.Options and moduleapi.Options['Favourite']
		local enabled = favopt ~= nil and favopt.Enabled == true
		if enabled then
			count = count + 1
			if not moduleapi.FavClone then
				pcall(function()
					...
					if object then
						...
						local function closeAllFavSettings(exceptModule)
							for _, m in pairs(self.Modules) do
								if m ~= exceptModule and m.Children and m.Children.Parent == favchildren then
									...
									m.Children.Parent = m.ChildrenParent or m.Children.Parent
								end
							end
						end
						local function openFavSettings()
							...
							if ch then
								if ch.Parent ~= favchildren then
									...
									ch.Visible = false
									ch.Parent = moduleapi.ChildrenParent or favchildren
								end
							end
						end
						...
						if favicon then
							favicon.MouseButton1Click:Connect(function()
								if moduleapi.Favourite then
									moduleapi.Favourite:Toggle()
								end
							end)
						end
						if bind then
							...
							bind.MouseEnter:Connect(function()
								...
								if bindicon then
									...
									bindicon.Image = getvapeasset('newvape/assets/new/edit.png')
									if not moduleapi.Enabled then bindicon.ImageColor3 = color.Dark(uipallet.Text, 0.16) end
								end
							end)
							bind.MouseLeave:Connect(function()
								if bindtext then bindtext.Visible = #(moduleapi.Bind and moduleapi.Bind.Keys or {}) > 0 end
								if bindicon then
							...
							bind.MouseButton1Click:Connect(function()
								self.Binding = moduleapi
							end)
						end
					end
				end)
			end
			local clone = moduleapi.FavClone
			if clone and clone.Parent then
				...
				if cloneFav and moduleapi.FavIcon then
					moduleapi.FavIcon.ImageColor3 = Color3.new(1, 1, 1)
				end
			end
		elseif moduleapi.FavClone then
			if moduleapi.Children and moduleapi.Children.Parent == favchildren then
				moduleapi.Children.Visible = false
				moduleapi.Children.Parent = moduleapi.ChildrenParent or favchildren
			end
			moduleapi.FavClone:Destroy()
			moduleapi.FavClone = nil
			moduleapi.FavOrder = nil
		end
	end
	if count == 0 and favcategory.Expanded then
		favcategory:Expand()
	end
end
...
general:CreateButton({
	...
	vape.Save = function() end
		if isfile('LarpV4/profiles/'..vape.Profile..vape.Place..'.txt') and delfile then
			delfile('LarpV4/profiles/'..vape.Profile..vape.Place..'.txt')
		end
...
function vape:Save(newProfile)
	...
	writefile('LarpV4/profiles/'..game.GameId..'.gui.txt', httpService:JSONEncode(guiData))
	writefile('LarpV4/profiles/'..self.Profile..self.Place..'.txt', httpService:JSONEncode(mainData))
end
...
components = {
			...
			if props.Profiles then
				...
				if index then
					if value ~= 'default' then
						...
						if isfile('LarpV4/profiles/'..value..vape.Place..'.txt') and delfile then
							delfile('LarpV4/profiles/'..value..vape.Place..'.txt')
						end
					end
				else
	...
	component.ChildrenParent = children
	local favbutton = Instance.new('ImageButton')
	favbutton.AutoButtonColor = false
	favbutton.BackgroundTransparency = 1
	favbutton.Name = 'Favourite'
	favbutton.Position = UDim2.new(1, -52, 0, 0)
	favbutton.Size = UDim2.fromOffset(22, 40)
	favbutton.Text = ''
	...
	favicon.Name = 'Icon'
	favicon.Position = UDim2.fromOffset(5, 14)
	favicon.Size = UDim2.fromOffset(12, 12)
	favicon.Parent = favbutton
	component.Favourite = component:CreateToggle({
		Name = 'Favourite',
		Default = false,
		Function = function(enabled)
			favicon.ImageColor3 = enabled and Color3.new(1, 1, 1) or color.Dark(uipallet.Text, 0.43)
			if vape.UpdateFavourites then vape:UpdateFavourites() end
		end
	})
	component.Favourite.Object.Visible = false
	favbutton.MouseButton1Click:Connect(function()
		component.Favourite:Toggle()
	end)
